    ...
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: const [
          ...
          BottomNavigationBarItem(icon: Icon(Icons.piano), label: 'Transcribir'),
  ...
  void initState() {
    ...
    _audioPlayer.positionStream.listen((position) {
      if (_selectedScore != null && _selectedScore!.duration > 0) {
        final double progress = position.inMilliseconds / (_selectedScore!.duration * 1000);
        if (!_playbackController.isAnimating && _isPlaying) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.transparent,
        elevation: 0,
        leading: const Icon(Icons.keyboard_arrow_down, color: Color(0xFFF8F9FA)),
        title: Column(
          children: [
            Text(
              ...
              Center(
                child: _selectedScore != null
                    ? ScoreStaveVisualizer(
                        score: _selectedScore!,
                        currentTime: _playbackController.value * (_selectedScore?.duration ?? 1.0),
              ...
              if (_selectedScore != null) ...[
                Row(
                  ...
                  children: [
                    ...
                    const Icon(Icons.favorite_border, color: Color(0xFFFF9800), size: 28),
                  ],
                ),
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          Text(
                            _formatTime(_playbackController.value * (_selectedScore?.duration ?? 0)),
                ...
                Row(
                  ...
                  children: [
                    IconButton(
                      icon: const Icon(Icons.stop, color: Color(0xFFE65100), size: 32),
                    ...
                    IconButton(
                      icon: const Icon(Icons.repeat, color: Color(0xFFFF9800), size: 24),
                      onPressed: () {},
                    ),
                  ],
                ),
                ...
                builder: (context, state) {
                  if (state is ScoreLibraryLoaded) {
                    ...
                    if (recentScores.isEmpty) {
                      return const Text('Aún no tienes partituras. ¡Ve a Transcribir!');
                    }
                    return Column(
                      children: recentScores.map((score) => _buildScoreItem(score)).toList(),
                    );
                  } else if (state is ScoreLibraryLoading) {
                    return const Center(child: CircularProgressIndicator());
                  } else {
                    return const Text('No se encontraron partituras recientes.');
                  }
                },
              ),
